...
4. **Venmo**: create an account with source "Venmo CSV import" and import a Venmo statement CSV the same way. Money you send counts as spending (category "Friends & Family" by default), money you receive is money in, and bank transfers are skipped entirely (they aren't imported at all). Duplicates are skipped by Venmo transaction ID, and pending or cancelled rows are not imported.
